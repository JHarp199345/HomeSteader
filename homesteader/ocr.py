"""Local macOS Vision OCR for image-only PDFs and phone image scans."""
...
def recognize_image_with_vision(source: Path) -> tuple[str, str | None]:
    """Read one local image without sending it to a cloud OCR service."""
    ...
    handler = VNImageRequestHandler.alloc().initWithURL_options_(NSURL.fileURLWithPath_(str(source)), None)
    try:
        success, error = handler.performRequests_error_([request], None)
    except Exception as error:
        return "", f"macOS Vision could not read the image: {error}"
    if not success:
        return "", f"macOS Vision could not read the image: {error or 'unknown error'}"
    text = "\n".join(lines).strip()
...
def recognize_pdf_with_vision(source: Path) -> tuple[str, str | None]:
    ...
    with TemporaryDirectory(prefix="homesteader-ocr-") as temporary_directory:
        ...
        for image_path in sorted(Path(temporary_directory).glob("page-*.png")):
            text, issue = recognize_image_with_vision(image_path)
            if issue and not text:
                return "", issue
            if text:
                pages.append(text)
    text = "\n\n".join(pages).strip()
